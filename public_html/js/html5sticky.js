var appName = "demo";
var url = 'https://api.masterdatanode.com/' + appName + '/sticky/';
...
html5sticky.addNote = function () {
    ...
    var SendInfo = {"Data": [{"identifier": nindex, "htext": $(stickynote).find('h2').text(), "ptext": $(stickynote).find('p').text(), "bgcolor": bgcolor, "pos": pos}]};
...
//            $("#test_div").html(JSON.stringify(result.description));
            html5sticky.showMessage('#9BED87', 'black', 'Sticky Note Added, update it by Clicking :)');
        },
...
html5sticky.saveNote = function (el) {
    if (Modernizr.localstorage) {
        var identifier = parseInt(html5sticky.getIdentifier($(el)));
...
//        bgcolor = localStorage.getItem(identifier + '|bgcolor');
        // get position info
//        pos = localStorage.getItem(identifier + '|pos');
        //alert(bgcolor);
        html5sticky.postCall(identifier, htext, ptext);
    } else {
        html5sticky.nohtml5();
    }
};
...
//                note_index = i;
                var stickynote, bgcolor, htext, ptext, pleft, ptop, temp_array, identifier;
                identifier = jsonObject.identifier;
                // get color and rotation level
                bgcolor = jsonObject.bgcolor;
                // get position info
                temp_array = jsonObject.pos.split('|');
                pleft = temp_array[0];
                ptop = temp_array[1];
                // get text info
                htext = jsonObject.htext;
                ptext = jsonObject.ptext;
                stickynote = $('<div class="note_common ' + bgcolor + '" />').appendTo($('#main'));
                // add tape to stickynote
                html5sticky.addPin(stickynote);
                $(stickynote).append($('<h2>' + htext + '</h2>'));
                $(stickynote).append($('<p>' + ptext + '</p>'));
                // append identifier
                $(stickynote).append($('<span id="idf_' + identifier + '" />'));
...
html5sticky.postCall = function (identifier, htext, ptext) {
...
////alert(name); 
    var SendInfo = {"Data": {"htext": htext, "ptext": ptext}, "filter": {"identifier": identifier}, "type": "single"};
    ////alert(JSON.stringify(SendInfo)); 
